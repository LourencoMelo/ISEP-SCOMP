...
int main(void){
    ...
    getchar();
    //Implementation------------------------------------------------
    sem_wait(sem_west); //block west direction
    sem_wait(sem_east); //only passing the cars that where in the sem
    ...
    sem_post(sem_east); //allows the same direction(priority)
    sem_post(sem_west); //allows the opposite direction
